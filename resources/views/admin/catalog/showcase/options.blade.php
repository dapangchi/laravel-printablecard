...
@if(isset($card))
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9">
            @if($ft->type == DROPDOWN)
                <?php $ftValue = ShowcaseOption::dropboxValue($showcase_id, $ft->id); ?> 
            ...
            @else
                @if($ft->both_side == 1)
                    ...
                    @foreach($ft->optionsSorted() as $i => $opt)
                        <label class="css-input css-checkbox css-checkbox-primary">
                            @if(ShowcaseOption::hasOption($showcase_id, $ft->id, $opt->id, FRONTSIDE))
                    ...
                    @foreach($ft->optionsSorted() as $i => $opt)
                        <label class="css-input css-checkbox css-checkbox-primary">
                            @if(ShowcaseOption::hasOption($showcase_id, $ft->id, $opt->id, BACKSIDE))
                ...
                @else
                     @foreach($ft->optionsSorted() as $i => $opt)
                        <label class="css-input css-checkbox css-checkbox-primary">
                            @if(ShowcaseOption::hasOption($showcase_id, $ft->id, $opt->id))
