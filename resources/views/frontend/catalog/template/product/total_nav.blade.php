        ...
        <div class="filter filter-card-type">
            ...
            <ul>
                @for($j = 0; $j < $nums; $j++)
                    ...
                    <li><a href="{{ CardTemplate::getSearchUrl($serviceType, ['card' => $cards[$i*$nums+$j]->slug]) }}">{{ $cards[$i*$nums+$j]->name }}</a></li>
                    @endif
                @endfor
            </ul>
            @endfor
        ...
        <div class="filter filter-style">
            ...
            <ul>
                @for($j = 0; $j < $nums; $j++)
                    ...
                    <li><a href="{{ CardTemplate::getSearchUrl($serviceType, ['tag' => $tags[$i*$nums+$j]->name]) }}">{{ $tags[$i*$nums+$j]->name }}</a></li>
                    @endif
                @endfor
            </ul>
            @endfor
